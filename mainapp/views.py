...
def transaction_list_create(request):
    if request.method == 'GET':
        transactions = Transactions.objects.filter(guide=request.user.guide_profile).order_by('-created_at')  # Correct reference
        ...
        # Filter services based on location names
        services = Services.objects.filter(location__in=locations)

        # Debugging: Check the retrieved services
        print("Filtered Services:", services)


        if not services.exists():
            return Response({"detail": "Invalid locations or no services available for those locations"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        data['service_ids'] = service_ids  # Add service IDs for saving

        
        user = get_object_or_404(UserProfile, user = request.user)
        
        print("request.user.user_profile.id ",user.id )
        # Assign the user and other necessary default values
        data['user_id'] = request.user.user_profile.id  # Correct user profile reference
        data['status'] = Transactions.StatusChoices.PENDING  # Default transaction status
        data['payment_status'] = False  # Default payment status
        guide = GuideProfile.objects.get(id=data['guide_id'])
        print("guide",guide)
        # Serialize the data and save the transaction
        serializer = TransactionSerializer(data=data)
       
        if serializer.is_valid():
            transaction = serializer.save()  # Create and save the transaction
            print("trsan", transaction)
            ...
            notification = Notification.objects.create(user = guide_user, title = "Request For Booking", message = "New booking request is placed")
            notification.save()
            user = FirebaseToken.objects.get(user = guide_user)
            print("user_token",user)
            send_firebase_notification(user.token,title = "Request For Booking",body = "New booking request is placed")
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        print(serializer.errors)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
